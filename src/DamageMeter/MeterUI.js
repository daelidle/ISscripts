class MeterUI {
    ...
    _updateMeter(players, meterType){
        ...
        let playersHtml = `<img class="dm_breakdown dm_icon_button" alt="Breakdown Damage Metrics" src="${this.breakdownIcon}">
                           <img class="dm_switcher dm_icon_button" alt="Switch between DPS/EDPS/Tanking/Healing Meters" src="${this.meterSwitchIcon}">`;
        ...
        const breakdown = document.getElementsByClassName("dm_breakdown")[0];
        breakdown.addEventListener("click",function(){
            window.damageMeter.showBreakdownModal();
        },false);
    }
    ...
    generateBreakdownModal(group, abilities){
        const title = "Damage Breakdown";
        const player = group[Object.keys(group)[0]];
